-- https://wiki.gtanet.work/index.php?title=Weapons_Models

-- /Notes
-- Doing the data this way is a bit limting
-- Could reate a list of all weapons and give it a 'category' id
-- To give the items order  in category, can give them a 'category_item_order' id and organize it like that
-- Then write a function to get all wepons from X category
-- That way the database is more dynamic - maybe

local weapons = {

    {
        name = "Melee",
        children = {

            {
                name = "Unarmed",
                hash = -1569615261
            },
            {
                name = "Knife",
                hash = -1716189206
            },
            {
                name = "Nightstick",
                hash = 1737195953
            },
            {
                name = "Hammer",
                hash = 1317494643
            },
            {
                name = "Bat",
                hash = -1786099057
            },
            {
                name = "Crowbar",
                hash = -2067956739
            },
            {
                name = "Golfclub",
                hash = 1141786504
            },
            {
                name = "Bottle",
                hash = -102323637
            },
            {
                name = "Dagger",
                hash = -1834847097
            },
            {
                name = "Hatchet",
                hash = -102973651
            },
            {
                name = "KnuckleDuster",
                hash = -656458692
            },
            {
                name = "Machete",
                hash = -581044007
            },
            {
                name = "Flashlight",
                hash = -1951375401
            },
            {
                name = "SwitchBlade",
                hash = -538741184
            },
            {
                name = "Poolcue",
                hash = -1810795771
            },
            {
                name = "Wrench",
                hash = 419712736
            },
            {
                name = "Battleaxe",
                hash = -853065399
            } 

        }
    },

    {
        name = "Handguns",
        children = {

            {        
                name = "Pistol",
                hash = 453432689
            },
            {
                name = "Pistol Mk2",
                hash = 3219281620
            },
            {
                name = "Combat Pistol",
                hash = 1593441988
            },
            {
                name = "Pistol 50",
                hash = -1716589765
            },
            {
                name = "SNS Pistol",
                hash = -1076751822
            },
            {
                name = "Heavy Pistol",
                hash = -771403250
            },
            {
                name = "Vintage Pistol",
                hash = 137902532
            },
            {
                name = "Marksman Pistol",
                hash = -598887786
            },
            {
                name = "Revolver",
                hash = -1045183535
            },
            {
                name = "AP Pistol",
                hash = 584646201
            },
            {
                name = "Stun Gun",
                hash = 911657153
            },
            {
                name = "Flare Gun",
                hash = 1198879012
            }

        }
    },

    {
        name = "Machine Guns",
        children = {
          
            {            
                name = "MicroSMG",
                hash = 324215364
            },
            {
                name = "Machine Pistol",
                hash = -619010992
            },
            {
                name = "SMG",
                hash = 736523883
            },
            {
                name = "SMG - Mk2",
                hash = 2024373456
            },
            {
                name = "Assault SMG",
                hash = -270015777
            },
            {
                name = "Combat PDW",
                hash = 171789620
            },
            {
                name = "MG",
                hash = -1660422300
            },
            {
                name = "Combat MG",
                hash = 2144741730
            },
            {
                name = "Combat MG - Mk2",
                hash = 3686625920
            },
            {
                name = "Gusenberg",
                hash = 1627465347
            },
            { 
                name = "MiniSMG",
                hash = -1121678507
            }

        }
    },

    {
        name = "Assault Rifles",
        children = {

            {
                name = "AssaultRifle",
                hash = -1074790547
            },
            {
                name = "AssaultRifleMk2",
                hash = 961495388
            },
            {
                name = "CarbineRifle",
                hash = -2084633992
            },
            { 
                name = "CarbineRifleMk2",
                hash = 4208062921
            },
            {
                name = "AdvancedRifle",
                hash = -1357824103
            },
            {
                name = "SpecialCarbine",
                hash = -1063057011
            },
            {
                name = "BullpupRifle",
                hash = 2132975508
            },
            {
                name = "CompactRifle",
                hash = 1649403952
            }

        }
    },

    {
        name = "Sniper Rifles",
        children = {

            {               
                name = "SniperRifle",
                hash = 100416529
            },
            {
                name = "HeavySniper",
                hash = 205991906
            },
            {
                name = "HeavySniperMk2",
                hash = 177293209
            },
            {
                name = "MarksmanRifle",
                hash = -952879014
            }

        }
    },

    {
        name = "Shotguns",
        children = {
            
            {            
                name = "PumpShotgun",
                hash = 487013001
            },
            {
                name = "SawnoffShotgun",
                hash = 2017895192
            },
            {
                name = "BullpupShotgun",
                hash = -1654528753
            },
            {
                name = "AssaultShotgun",
                hash = -494615257
            },
            {
                name = "Musket",
                hash = -1466123874
            },
            {
                name = "HeavyShotgun",
                hash = 984333226
            },
            {
                name = "DoubleBarrelShotgun",
                hash = -275439685
            },
            {
                name = "Autoshotgun",
                hash = 317205821
            }

        }
    },

    {
        name = "Heavy Weapons",
        children = {
                
            {
                name = "GrenadeLauncher",
                hash = -1568386805
            },
            {
                name = "RPG",
                hash = -1312131151
            },
            {
                name = "Minigun",
                hash = 1119849093
            },
            {
                name = "Firework",
                hash = 2138347493
            },
            {
                name = "Railgun",
                hash = 1834241177
            },
            {
                name = "HomingLauncher",
                hash = 1672152130
            },
            {
                name = "GrenadeLauncherSmoke",
                hash = 1305664598
            },
            {
                name = "CompactLauncher",
                hash = 125959754
            }

        }
    },

    {
        name = "Thrown Weapons",
        children = {

            {     
                name = "Grenade",
                hash = -1813897027
            },
            {
                name = "StickyBomb",
                hash = 741814745
            },
            {
                name = "ProximityMine",
                hash = -1420407917
            },
            {
                name = "BZGas",
                hash = -1600701090
            },
            {
                name = "Molotov",
                hash = 615608432
            },
            {
                name = "FireExtinguisher",
                hash = 101631238
            },
            {
                name = "PetrolCan",
                hash = 883325847
            },
            {
                name = "Flare",
                hash = 1233104067
            },
            {
                name = "Ball",
                hash = 600439132
            },
            {
                name = "Snowball",
                hash = 126349499
            },
            {
                name = "SmokeGrenade",
                hash = -37975472
            },
            {
                name = "Pipebomb",
                hash = -1169823560
            }

        }
    }

}

return weapons